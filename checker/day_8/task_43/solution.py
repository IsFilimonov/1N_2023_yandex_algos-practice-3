...
index - последний обработанный заказ

first - время, когда курьер 1 закончил свою часть заказов

second - минимальное время, когда курьер 2 закончил

как вычислить dp[index][first]?

- либо index-заказ выполнил курьер 1

тогда у нас dp[index][first] <- dp[index - 1][first - a[i]]

- либо index-заказ выполнил курьер 2

тогда у нас dp[index][first] <- dp[index - 1][first] + b[i]
