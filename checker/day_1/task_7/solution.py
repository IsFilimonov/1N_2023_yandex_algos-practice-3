...
# для корректности вычисления добавляем 24 часа, если время приема перешло в новые сутки
# отправка: 21:00, прием: 06:00, разница: 24+6-21=9
if h3 < h1: h3 += 24
...
# s_lat =  int( ( abs(s3-s1) + abs(m3-m1)*60 + abs(h3-h1)*3600 ) / 2 + 0.5)

# До конца суток на основном сервере
s_orig_day_end = (23*3600+59*60+59) - (h1*3600+m1*60+s1)

# До конца суток на сервере SNTP: от времени на SNTP до 86399 секунд (день без одной секунды)
s_SNTP_day_end = (23*3600+59*60+59) - (h2*3600+m2*60+s2)

answer = ""

# Могут ли из-за сетевой задержки время SNTP перейти на новые сутки?
if s_lat > s_SNTP_day_end:
    # Наступают новые сутки на SNTP
    s_lat -= s_SNTP_day_end + 1
    hh = s_lat//3600
    s_lat -= hh*3600
    mm = s_lat//60
    s_lat -= mm*60
